...
static void		ft_printline_x(t_line *line, t_info *e)
{
	int	i;
	int	cumul;
	...
	while (i <= ABS(line->dx))
	{
		...
		mlx_pixel_put(e->mlx, e->win, e->tr + line->x, e->td + line->y,
			get_color(line->cp1, line->cp2, e->is_decr * i, line->dx));
		i++;
	}
}

static void		ft_printline_y(t_line *line, t_info *e)
{
	int	i;
	int	cumul;
	...
	while (i <= ABS(line->dy))
	{
		...
		mlx_pixel_put(e->mlx, e->win, e->tr + line->x, e->td + line->y,
			get_color(line->cp1, line->cp2, e->is_decr * i, line->dy));
		i++;
	}
}

static void		ft_join_points(t_info *e, t_point *p1, t_point *p2)
{
	t_line	line;
...
static void		ft_before_draw(t_info *e, t_point **points, int i)
{
	int		j;
	int		k;
	t_point	nullpt;
