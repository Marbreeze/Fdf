...
/*   Updated: 2019/08/18 22:11:30 by jinpark          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		create_isometric_2(t_fdf *fdf, int *row, int column)
{
	...
	{
		column = 0;
		if (!(fdf->triple_iso_mat[*row] = (t_point **)malloc(sizeof(t_point*) * (fdf->column + 1))))
			return ;
		while (column < fdf->column)
		{
			if (!(fdf->triple_iso_mat[*row][column] = (t_point *)malloc(sizeof(t_point))))
				return ;
			fdf->triple_iso_mat[*row][column]->x = 400 + ((fdf->triple_matrix[*row][column]->x
			- fdf->triple_matrix[*row][column]->y) * cos(0.523599));

			fdf->triple_iso_mat[*row][column]->y = 150 + (((fdf->triple_matrix[*row][column]->x
			+ fdf->triple_matrix[*row][column]->y) * sin(0.523599)) 
			- fdf->triple_matrix[*row][column]->z);
			
			fdf->triple_iso_mat[*row][column]->z = fdf->triple_matrix[*row][column]->z;
			column++;
		}
